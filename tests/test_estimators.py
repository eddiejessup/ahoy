...
class TestLinearSpatialCDotEstimators(test.TestBase):
    ...
    def run_nd(self, dim, u_0, cdots_expected):
        L = dim * [1.0]
        ...
        self.assertTrue(np.allclose(cdots, cdots_expected))

    def run_parallel_nd(self, dim):
        u_0 = np.zeros([self.n, dim])
        u_0[:, 0] = 1.0
        self.run_nd(dim, u_0, self.v_0)
...
class TestLinearTemporalCDotEstimators(TestLinearSpatialCDotEstimators):
